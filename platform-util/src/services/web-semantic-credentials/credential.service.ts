...
const createCredential =
  (auth: IAuth) =>
  async (args: CreateCredentialArgs): Promise<CreateCredentailReqResponse> => {
...
const retrieveCredentials =
  (auth: IAuth) =>
  async (args: RetrieveCredentialsArgs): Promise<CreateCredentailReqResponse[]> => {
    let url: string;
    switch (args) {
      case undefined:
