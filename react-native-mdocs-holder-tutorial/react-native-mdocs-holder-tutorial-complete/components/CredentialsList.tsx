...
export default function CredentialsList({ mobileCredentials, deleteMobileCredential }: CredentialsListProps) {
    ...
    <View style={styles.credentialsContainer}>
      ...
      <ScrollView style={styles.scrollView}>
        {mobileCredentials.map((credential, index) => (
          <Collapsible key={credential.id || index} title={credential.branding?.name || `Credential ${index + 1}`}>
            <View style={styles.credentialDetails}>
              <TouchableOpacity style={styles.deleteButton} onPress={() => deleteMobileCredential(credential.id)}>
                <Text style={styles.buttonText}>Delete Credential</Text>
              </TouchableOpacity>
              <Text style={styles.credentialText}>{JSON.stringify(credential, null, 2)}</Text>
            </View>
          </Collapsible>
        ))}
      </ScrollView>
