 ...
 *
 * Import the `MobileCredentialResponse` type from the SDK. This type holds the verification results
 * returned by your MATTR VII tenant after the remote (app-to-app) presentation completes, including:
 * - `credentials`: Array of verified credential presentations, each containing the docType, claims
 *   (organized by namespace), verification result, and any claim errors.
 * - `credentialErrors`: Any top-level errors that occurred during verification.
 */
...
/**
 * Step 4: Verification results modal
 *
 * Displays the results of the remote mDoc verification to the verifier app user.
 *
 * In the remote presentation workflow the MATTR VII tenant performs the verification server-side and
 * returns the results to the SDK, which surfaces them as a `MobileCredentialResponse`. The verifier
 * does not need any on-device trust anchors: trusted issuers are configured on the tenant.
 *
 * The modal displays:
 ...
 * 5. Claim errors - claims that were requested but not provided.
 *
 * Props:
        ...
        <ScrollView style={styles.content}>
          {verificationResults.credentials && verificationResults.credentials.length > 0 ? (
            <View>
              ...
              {/* Claims organized by namespace. Each mDoc credential contains claims grouped under
                  namespaces (e.g. "org.iso.18013.5.1" for standard mDL fields). The claims shown here
                  correspond to what was requested in the MobileCredentialRequest. */}
              {verificationResults.credentials.map((credential, credIndex) => (
                <View key={`credential-${credIndex}`}>
                  ...
                  {/* Claim errors occur when the presentation completed successfully, but some requested
                      claims were not provided (absent from the mDoc, or consent declined). */}
